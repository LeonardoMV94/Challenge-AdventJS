...
function daysToXmas(date) {
    ...
    const dias = ((navidad.getTime() - date.getTime()) / (1000 * 60 * 60 * 24))   // to miliseconds and subtract

    if( dias > 0 ) return Math.ceil(dias)
    if( dias < 0 ) return parseInt(dias)

    return dias
}
const date10 = new Date('Dec 1, 2021')
console.log(daysToXmas(date10)) // 24
